...
def _encode_trits(seq: str, *, use_carry: bool = True, t_bonus: bool = True) -> list[int]:
    """MCORE-style scan with optional carry propagation and T increment."""
    trits: list[int] = []
    carry = 0
    for base in seq.upper():
        if base not in _BASE_VAL:
            continue
        val = _BASE_VAL[base] + (carry if use_carry else 0) + (1 if (t_bonus and base == "T") else 0)
        trits.append(val % 3)
        carry = val // 3 if use_carry else 0
    return trits


def _aligned_trit_streams(
    wt_dna: str,
    mut_dna: str,
    *,
    use_carry: bool = True,
    ...
    n = min(len(wt_dna), len(mut_dna))
    return _encode_trits(wt_dna[:n], use_carry=use_carry, t_bonus=t_bonus), _encode_trits(
        mut_dna[:n], use_carry=use_carry, t_bonus=t_bonus
    )


def _mismatch_stats_from_trits(
    wt_trits: list[int],
    mut_trits: list[int],
    deletion_pos_1: int,
) -> dict[str, float | int]:
    return _carry_propagation_stats(wt_trits, mut_trits, deletion_pos_1)


def _control_densities(wt_dna: str, mut_dna: str, deletion_pos_1: int) -> dict[str, float]:
    """Downstream mismatch densities under four comparison models."""
    plain = _plain_frameshift_stats(wt_dna, mut_dna, deletion_pos_1)["density_after_site"]
    wt_c, mut_c = _aligned_trit_streams(wt_dna, mut_dna, use_carry=True, t_bonus=True)
    wt_nc, mut_nc = _aligned_trit_streams(wt_dna, mut_dna, use_carry=False, t_bonus=True)
    wt_nt, mut_nt = _aligned_trit_streams(wt_dna, mut_dna, use_carry=False, t_bonus=False)
    carry = float(_mismatch_stats_from_trits(wt_c, mut_c, deletion_pos_1)["density_after_site"])
    no_carry = float(_mismatch_stats_from_trits(wt_nc, mut_nc, deletion_pos_1)["density_after_site"])
    no_tbonus = float(_mismatch_stats_from_trits(wt_nt, mut_nt, deletion_pos_1)["density_after_site"])
    return {
        "plain_dna": float(plain),
        "trit_no_carry": no_carry,
        "mcore_carry": carry,
        "no_t_bonus": no_tbonus,
    }
...
def main() -> None:
    ...
    def positional_block(
        ...
        ctrl = _control_densities(wt_dna, mut_dna, del_pos_1)
        return {
            "window_start": wstart,
            "window_chi2": chi_w,
            "window_p": p_w,
    ...
    print("\nDownstream mismatch densities (prefix-aligned, after deletion site)")
    print("  Model columns: plain_DNA | trit_no_carry | MCORE_carry | no_T_bonus")
    for key, name, dpos in (
        ...
        r = pos_results[key]
        print(
            f"  {name}: first_diff={r['carry_first_diff_1based']}, "
            f"{r['plain_dna']:.3f} | {r['trit_no_carry']:.3f} | {r['mcore_carry']:.3f} | {r['no_t_bonus']:.3f}"
        )
    ...
    controls_path = FIG_DIR / "controls_table.tex"
    with open(controls_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "\\caption{Downstream mismatch density $\\rho$ under four comparison models "
            "(disagreements strictly after the deletion coordinate). "
            "Plain DNA uses same-index nucleotides; trit models use independent "
            "prefix re-encoding with carry on/off and with/without the $T$-specific increment.}\n"
        )
        f.write("\\begin{tabular}{lrrrr}\n")
        f.write("\\toprule\n")
        f.write(
            "Variant & Plain DNA $\\rho$ & Trit, no carry $\\rho$ & MCORE carry $\\rho$ "
            "& No $\\varepsilon_T$ $\\rho$ \\\\\n"
        )
        f.write("\\midrule\n")
        for label, key in (("c.35delG", "c35"), ("c.235delC", "c235")):
            r = pos_results[key]
            f.write(
                f"{label} & {r['plain_dna']:.3f} & {r['trit_no_carry']:.3f} & "
                f"{r['mcore_carry']:.3f} & {r['no_t_bonus']:.3f} \\\\\n"
            )
        f.write("\\bottomrule\n")
        f.write("\\end{tabular}\n")
        f.write("\\end{table}\n")
    ...
    with open(stats_tex, "w", encoding="utf-8") as f:
        ...
        f.write(f"\\newcommand{{\\PlainDensCThirtyFive}}{{{r35['plain_dna']:.3f}}}\n")
        f.write(f"\\newcommand{{\\PlainDensCTwoThirtyFive}}{{{r235['plain_dna']:.3f}}}\n")
        f.write(f"\\newcommand{{\\NoCarryDensCThirtyFive}}{{{r35['trit_no_carry']:.3f}}}\n")
        f.write(f"\\newcommand{{\\NoCarryDensCTwoThirtyFive}}{{{r235['trit_no_carry']:.3f}}}\n")
        f.write(f"\\newcommand{{\\NoTBonusDensCThirtyFive}}{{{r35['no_t_bonus']:.3f}}}\n")
        f.write(f"\\newcommand{{\\NoTBonusDensCTwoThirtyFive}}{{{r235['no_t_bonus']:.3f}}}\n")
